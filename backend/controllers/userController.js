...
const bookAppointment = async (req, res) => {
    try {
        ...
        if(userData.status !== UserStatus.ACTIVE){
            return res.json({ success: false, message: "You are not allowed to book an appointment, please fill you profile data" });
        }
...
//             return res.json({
//                 success: false,
//                 message: "You are not allowed to book an appointment, please fill your profile data",
//             });
//         }
...
// // ✅ دالة جديدة لإضافة تقييم من المستخدم
...
//             return res.json({ success: false, message: "نص التقييم مطلوب" });
//         }

//         const user = await userModel.findById(userId).select("name image address");
//         if (!user) {
...
//             userLocation: user.address?.line1 || "",
//             text,
//         };

//         // تخزين التقييم داخل المستخدم (أو يمكن لاحقاً إنشاء testimonialModel منفصل)
//         user.testimonial = testimonial;
//         await user.save();

//         res.json({ success: true, message: "تمت إضافة التقييم" });
//     } catch (error) {
